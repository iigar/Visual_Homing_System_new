...
### D-025: Pi hardware capture — двійний гейт + async→sync міст, fail-closed (M11)
**Рішення:** `PiCameraSource : ICameraSource` (pimpl ховає libcamera headers від public API). Два гейти: (1) compile-time `VH_ENABLE_LIBCAMERA` (PUBLIC define, бо умовно оголошує pimpl member — ABI consistency) — desktop компілює fail-closed stub без libcamera-залежності, `open()` → `NotCompiledIn`; (2) runtime — `open()` має успішно acquire camera + сконфігурувати R8 stream у capture dims, інакше source лишається closed і `next_frame()` → nullopt. Config validate (dims∈(0,8192], buffers≥1, timeout>0) передує compile-гейту (InvalidConfig однаково на desktop/Pi). libcamera async (`requestCompleted` на власному потоці) → синхронний `next_frame()` через mutex+condition_variable+черга completed requests, ізольовано в backend (pipeline зовні single-threaded). Капче у capture dims (raw), препроцес далі → target dims; копіювання row-by-row з урахуванням `StreamConfiguration::stride`.
**Чому:** промпт M11. Перший milestone на залізі → fail-closed скрізь. Pimpl тримає libcamera з public header, тож desktop тести не лінкують libcamera. Validate-перед-гейтом — детермінована поведінка незалежно від платформи.
**SAFETY:** `VH_ENABLE_LIBCAMERA` НЕ передбачає жодного live output — `build-test-pi.sh` тримає всі live-output опції явно OFF, тільки libcamera ON. Будь-яке відхилення формату/розмірів від запиту профілю після `validate()` → `ConfigureFailed` (не приймаємо adjusted geometry мовчки).
**Крос-валідація:** libcamera 0.3+ API (Trixie) підтверджено NotebookLM 2026-06-16 (formats::R8, plane.fd().get() SharedFD, stride row-copy). ⚠ Pi-секція (`#if VH_ENABLE_LIBCAMERA`) НЕ компілювалась цю сесію (немає Pi) — обовʼязково `build-test-pi.sh` на реальному Pi перед польовим використанням.
**How to apply:** M12 (live dry-run matching) бере кадри з `PiCameraSource` на Pi; capture dims з активного CameraProfile (M10). Thermal — окремий milestone (libcamera його не покриває).

### D-026: Live route matching dry-run — speed mismatch як явна змінна, endpoint stop, live-output hard-blocked (M12)
**Рішення:** `DryRunMatchSession` над `DryRunBridge` (M9). Per-frame: matcher (M5) → RouteMatch → bridge.tick (navigator M7 + telemetry-health gate M8 + dry-run sink M9) → акумуляція evidence. Швидкість запису vs відтворення = явна validation-змінна: трекаємо progress first/last/min/max, рахуємо regressions + rollback magnitude (mille) + index jumps (|Δindex|>1). ExpectedProgress any/forward/reverse визначає що є "backward". Endpoint gate (forward: progress≥gate; reverse: ≤1000−gate): при перетині — STOP генерації команд, `stop_reason=endpoint_reached`, наступні кадри без команд (fail-closed, контроль назад до autopilot boundary). `step_match(RouteMatch)` для тестів, `step(Frame)` матчить через matcher. Compact log (`format_compact_log`): passed, frames=X/Y, effective/configured fps, elapsed, valid_matches, progress range, regressions/rollback/jumps, endpoint_passed, progress_gate_passed, confidence_min_avg, telemetry_health/dropped, dry_run_quality, dry_run_valid=X/Y, live_output_gate_allowed/blocked/reasons, stop_reason. CLI `vh_match_session`.
**Чому:** промпт M12. Складаємо вже перевірені компоненти в evidence-цикл; жодної нової команд-логіки. Speed mismatch видно в логу, а не діє мовчки.
**SAFETY:** live-output межа hard-closed (немає M13 gate, немає M16/M17 writer) — кожен command attempt рахується BLOCKED з reason `live_output_disabled`, allowed завжди 0. dry_run_quality (M6 verdict) — окремий gate: false → passed=false навіть якщо все інше зелене. telemetry stale → bridge блокує валідність + telemetry_dropped++.
**How to apply:** session матчить кадри ВЖЕ у route/target dims — preprocess (M2) — задача pipeline вище (PiCameraSource капче capture dims → preprocess → target). M13 додасть LiveMavlinkOutputSafetyGate; M14 — readiness checker над compact log.

### D-027: Live-output safety scaffolding — gate/audit/session/bridge, default-blocked, нуль transmission (M13)
**Рішення:** 4 fail-closed компоненти, жодного live output. `LiveMavlinkOutputSafetyGate.evaluate(SafetyGateInputs)` → `GateDecision{allowed, block_reasons[]}`, allowed лише коли reasons порожні; детермінований порядок 15 reasons: runtime_not_enabled, operator_not_confirmed, writer_not_owned, audit_not_ready, dry_run_quality_not_passed, camera_frame_timeout (frames_seen=0 / frame_age>500ms / <0), telemetry_stale (!mavlink_ok), vehicle_not_armed, match_invalid, match_stale (age<0/>200ms), match_low_confidence (<800), command_invalid, command_not_finite (isfinite yaw/vx/vy), command_out_of_bounds (|yaw_microradps|>1e6), forward_speed_nonzero (vx/vy≠0 — exact zero для першого scope). Default config (runtime/operator OFF) → завжди blocked. `LiveMavlinkOutputAuditLog : IAuditLog` — record start/decision/stop, `can_write = ready && !write_should_fail`; not ready / write fail → record повертає false. `LiveMavlinkBridge : ICommandSink` — stub, available()/start()/send()/started() усі false (real writer = M17 за VH_ENABLE_WRITER_ATTACH). `LiveMavlinkOutputSession` координатор: start() fail-closed (audit ready + record_start), tick() → evaluate → audit decision (write fail → +audit_write_failed, stop, нічого не маршрутизує) → dry_sink.send (history only) → allowed: live_bridge.send рефузить (live_rejected++, нуль TX); block_reason_counts агрегує (reason→count для M14). mark_endpoint() → record_stop("endpoint_progress_reached") + stop generation; stop(reason) final record.
**Чому:** промпт M13. Gate потребує більше входів ніж ISafetyGate::evaluate(cmd,telem,health) → власний SafetyGateInputs (+ownership/audit/quality/match). AuditLog реалізує наявний IAuditLog. Bridge ICommandSink-shaped але рефузить.
**SAFETY:** жоден шлях не передає команду. Навіть gate-allowed рішення впирається в LiveMavlinkBridge що рефузить (live_rejected). Audit write failure = block + stop session (autopilot тримає контроль). CMake ланцюг підтверджено fail-closed: WRITER_ATTACH потребує LIVE_OUTPUT+BENCH_PROPS_OFF, LIVE_OUTPUT потребує BENCH_PROPS_OFF — partial config = FATAL_ERROR.
**How to apply:** M14 readiness checker grep'ає block_reason (bench scenario: усе зелене крім armed → єдина причина vehicle_not_armed:N). M16/M17 додадуть реальний writer за gate+audit+bench props-off chain.

### D-028: Readiness checkers + evidence — лог-формат true/false, gate-derived reason:count, чесний vehicle_not_armed (M14)
**Рішення:** 3 shell checkers над evidence-логами. (1) Гармонізація формату: `format_compact_log` bool → `true/false` (не 1/0), block reasons → `reason:count` (map-sorted) — щоб `check-live-readiness-log.sh` grep'ав `passed=true`/`vehicle_not_armed:N` як вимагає промпт. (2) Інтеграція M13 gate у DryRunMatchSession: `set_live_output_gate(gate, LiveOutputContext{single_writer_owned, audit_ready})` — per-frame будує SafetyGateInputs (health+telemetry з bridge+match+command+now) → gate.evaluate → реальні allowed/blocked + reason counts (без gate: fallback `live_output_disabled`). (3) AuditRecord +command_valid+vx_mps; `LiveMavlinkOutputAuditLog::format_log()` — one event/line. (4) CLI: `vh_match_session --readiness` (gate operator-confirmed + disarmed fresh heartbeat → vehicle_not_armed:N), новий `vh_live_session` (драйвить LiveMavlinkOutputSession → audit log, endpoint→mark_endpoint). (5) `check-live-readiness-log.sh` + `check-live-session-audit-log.sh` (параметризовані expected count, дефолт 150). (6) `docs/LIVE_OUTPUT_READINESS_RECORD.md` — ledger для 3/3 Pi evidence.
**Чому:** промпт M14. Readiness log = M12 compact log → формат мусить точно матчити чекер. vehicle_not_armed має бути ЧЕСНИМ (реальний gate на disarmed bench), не штампом — тому інтеграція gate, не hard-code.
**SAFETY:** evidence completion ≠ дозвіл на live output (явно в RECORD.md + промпт). Endpoint gate=1000 у evidence run → endpoint точно на фінальному кадрі (150 decisions + 1 stop, без post-endpoint tail). E2E desktop (150-кадровий self-replay): readiness+audit обидва pass, live_output_gate_allowed=0, нуль TX.
**How to apply:** 3/3 Pi evidence збираються на M15 (після M11 Pi bring-up). Desktop reference у RECORD.md доводить tooling, не 3/3 Pi стан. M15 = повний pre-live state; live output лишається blocked.
